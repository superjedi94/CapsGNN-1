...
def drop_channel(inputs,dropout_rate,name,if_train):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        inputs = tf.keras.layers.Dropout(rate=dropout_rate,name=name, noise_shape=[tf.shape(inputs)[0], tf.shape(inputs)[1],tf.shape(inputs)[2],1])(inputs, training=if_train)  # (batch, N, C, d)
    return inputs
...
def node_embedding_gen(inputs,trans_mats,N,in_channel, out_channel,node_embedding_size, name):
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        nets = tf.keras.layers.Conv2D(
            filters=node_embedding_size * out_channel,
            kernel_size=1,
            strides=1, padding='VALID', use_bias=True)(inputs)  # (?, N, Co*d)
    ...
    tf.compat.v1.logging.info("{} output shape: {}".format(name, nets.get_shape()))
    return nets
...
def nn_attention_layer(inputs,batch_size,mask,name,emb_size,channel_num):
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        atten = tf.compat.v1.layers.dense(inputs_, units=int(emb_size * channel_num / 16), activation=tf.nn.tanh)  # (?*N, C*d/16)
        atten = tf.compat.v1.layers.dense(atten, units=channel_num, activation=None)  # (?*N, C)
        ...
        num_nodes = tf.expand_dims(tf.reduce_sum(mask, axis=1, keepdims=True), axis=-1)
...
def capsule_layer(inputs, Ci, Co, in_emb_size, graph_emb_size, iterations, position_emb_size,nodes_indicator, batch_size, coordinate, name):
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        with tf.compat.v1.variable_scope('votes') as scope:
            ...
            tf.compat.v1.logging.info("{} votes shape: {}".format(name, votes.get_shape()))

        with tf.compat.v1.variable_scope('routing') as scope:
            num_nodes = tf.reduce_sum(nodes_indicator, axis=1, keepdims=True)  # (?, #1, 1)
...
def loss_layer(inputs,reconstruct_value,label,lambda_val,batch_size,num_class,reconstruct_num,name,reg_scale):
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        with tf.compat.v1.variable_scope('margin_loss') as scope:
        ...
        with tf.compat.v1.variable_scope('recons_loss') as scope:
            ...
            decoded = tf.compat.v1.layers.dense(correct_output,units= 128)
            decoded = tf.compat.v1.layers.dense(decoded,units = reconstruct_num)
            ...
            reconstruct_value_max = tf.reduce_max(reconstruct_value,axis=1,keepdims=True)
...
def mask_softmax(inputs,mask,dim):
    ...
    """
    with tf.compat.v1.variable_scope('bulid_mask') as scope:
        ...
        sum_col = tf.reduce_sum(masked_e_inputs,axis=dim,keepdims=True) +epsilon  # (batch, 1, C, 1)
        result = tf.compat.v1.div(masked_e_inputs,sum_col)  # (batch, N, C, 1)
    return result
...
def adj_mats_normalize(batch_adj_mats, node_indicator):
    ...
    with tf.compat.v1.variable_scope('adj_mats_normal') as scope:
        row_sum = tf.math.reduce_sum(batch_adj_mats,axis=-1,keepdims=True)  # (batch , N, 1)
        ...
        r_mat_inv = tf.linalg.diag(r_inv)  # (batch , N, N)
...
def routing_graph(votes,b_Ij,num_nodes, iterations = 3):
    ...
    for r_iter in range(iterations):
        with tf.compat.v1.variable_scope('iter_' + str(r_iter)) as scope:
            c_ij = tf.nn.softmax(b_Ij, axis=4)  # (?, 1, 1, Ci, Co, 1)
            if r_iter == iterations - 1:
                s_j = tf.multiply(c_ij, u_hat)
                s_j = tf.reduce_sum(s_j, axis=3, keepdims=True)/num_nodes
...
def squash(v_j, dim = -1):
    ...
    :return:
    """
    vec_squared_norm = tf.reduce_sum(tf.square(v_j), dim, keepdims=True)
